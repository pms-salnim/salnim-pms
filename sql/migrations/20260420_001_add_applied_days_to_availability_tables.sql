...
ADD COLUMN applied_days JSONB DEFAULT NULL;
...
-- 2. Add applied_days to restrictions table
ALTER TABLE restrictions 
ADD COLUMN applied_days JSONB DEFAULT NULL;

COMMENT ON COLUMN restrictions.applied_days IS 'Days of week this restriction applies to (array of 0-6). NULL means all days. This complements the existing days_of_week field for OTA sync compatibility.';

CREATE INDEX IF NOT EXISTS idx_restrictions_applied_days 
ON restrictions USING GIN (applied_days);
...
ON restrictions(property_id, date_range_start, date_range_end) 
WHERE applied_days IS NOT NULL;
