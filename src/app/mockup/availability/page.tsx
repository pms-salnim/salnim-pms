...
const App = () => {
  ...
  const [gridData, setGridData] = useState<{ [key: string]: GridDataCell }>({});
  ...
  // --- INITIALIZATION ---
  useEffect(() => {
    const initialData: { [key: string]: GridDataCell } = {};
  ...
  const getCellData = (rowId: string, date: Date): GridDataCell => {
    const dateKey = date.toISOString().split('T')[0];
  ...
  const handleMouseDown = (id: string, date: Date) => {
    const key = `${id}|${date.toISOString().split('T')[0]}`;
    setIsDragging(true);
    setSelectedCells(new Set([key]));
  };

  const handleMouseEnter = (id: string, date: Date) => {
  ...
  const saveCellEdit = (newData: Partial<GridDataCell>) => {
    ...
    setGridData((prev) => ({
      ...
      [key]: {
        ...prev[key],
        ...newData,
        isOverride: !editingCell.isRoomLevel,
      },
    }));
    setEditingCell(null);
  };
  ...
  const Cell = ({
    ...
    // For room type cells, use the base price from the room type if not overridden
    const displayData = !isRoomLevel && roomTypeBasePrice > 0 && data.price === 0 
      ? { ...data, price: roomTypeBasePrice }
      : data;
        ...
        >
          {data.status === 'available' ? (
            ...
            <div className="flex flex-col items-center gap-0.5">
              ...
              <span className="text-[8px] font-bold text-rose-600 uppercase tracking-tight">
                Stop Sell
              </span>
            </div>
          )}
    ...
    return (
      <div
        ...
        onMouseEnter={() => handleMouseEnter(rowId, date)}
        onClick={(e) => {
          if (e.detail === 2) openEditor(rowId, date, data, false);
        }}
        className={`
            ...
            <div className="mb-1.5">
              <span
                className={`text-[15px] font-bold leading-none ${
                  displayData.isOverride ? 'text-blue-600' : 'text-slate-800'
                }`}
              >
            ...
            <div className="flex flex-col gap-1 mt-auto">
              <div className="flex items-center gap-1">
                <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200 font-medium whitespace-nowrap">
                  Min {displayData.minStay} night
                </span>
                {displayData.cta && (
                  <span className="text-[9px] bg-amber-100 text-amber-700 px-1 py-0.5 rounded border border-amber-200 font-bold">
                    CTA
                  </span>
                )}
                {displayData.ctd && (
        ...
        {selectedCells.size > 0 && (
          <button
            ...
            onClick={() => {
              ...
              const [rowId, dateKey] = cellKey.split('|');
            ...
            <tr className="bg-white border-b">
              ...
              {DATES.map((date, idx) => (
                <th key={idx} className="min-w-[160px] p-2 text-center border-r border-b bg-white">
                  <div className="text-[10px] font-medium text-slate-400 uppercase leading-none mb-1">
                ...
                <tr className="group">
                  ...
                  {DATES.map((date, idx) => (
                    <td key={idx} className="p-0 border-b">
                      <Cell rowId={rt.id} date={date} isRoomLevel={false} roomTypeBasePrice={rt.basePrice} />
                    </td>
                  ))}
                </tr>

                {expandedRows[rt.id] &&
                  Array.from({ length: rt.units }).map((_, i) => (
                    <tr key={`${rt.id}-room-${i}`} className="bg-slate-50/50">
                      <td className="sticky left-0 z-10 bg-slate-50 border-r border-b pl-10 pr-3 py-2 shadow-[2px_0_5px_rgba(0,0,0,0.01)]">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[8px] uppercase text-slate-400 font-bold tracking-tight mt-1">
                            Standard
                          </span>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in duration-150">
            <div className="p-5 border-b flex justify-between items-center">
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-medium uppercase">
                  {editingCell.date.toLocaleDateString('en-US', {
                    dateStyle: 'medium',
                  })}
                </p>
              </div>
              ...
              {!editingCell.isRoomLevel && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      type="number"
                      defaultValue={editingCell.data.price}
                      id="edit-p"
                      className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-md font-bold focus:ring-2 focus:ring-blue-500 outline-none"
                    />
                  </div>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() =>
                      setEditingCell((prev) =>
                        prev
                          ? { ...prev, data: { ...prev.data, status: 'available' } }
                          : null
                      )
                    }
                    className={`py-2 px-3 rounded-lg border-2 flex items-center justify-center gap-2 text-xs font-bold transition-all ${
                      editingCell.data.status === 'available'
                    ...
                    onClick={() =>
                      setEditingCell((prev) =>
                        prev
                          ? {
                              ...prev,
                              data: { ...prev.data, status: 'stop-sell' },
                            }
                          : null
                      )
                    }
                    className={`py-2 px-3 rounded-lg border-2 flex items-center justify-center gap-2 text-xs font-bold transition-all ${
                      editingCell.data.status === 'stop-sell'
            ...
            <div className="p-5 bg-slate-50 flex gap-2">
              ...
              <button
                onClick={() => {
                  const priceInput = document.getElementById('edit-p') as HTMLInputElement | null;
                  saveCellEdit({
                    ...editingCell.data,
