...
const StaffManagementPage = () => {
  const { user: currentUser, property, isLoadingAuth } = useAuth();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      toast({ title: t('toasts.error_title'), description: "Could not fetch staff members.", variant: "destructive" });
      setIsLoading(false);
    });

    return () => unsubscribe();
  }, [currentUserPropertyId, t]);
  ...
  const handleToggleStatusStaff = async (staffId: string, currentStatus: StaffStatus) => {
     try {
        ...
        toast({ title: t('toasts.success_title'), description: t('toasts.status_updated', { status: t(`statuses.${newStatus.toLowerCase()}`) }) });
     } catch (error) {
        toast({ title: t('toasts.error_title'), description: t('toasts.status_update_error'), variant: "destructive" });
     }
  };
