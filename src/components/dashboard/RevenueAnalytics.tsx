    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-6">
          ...
          {/* Advanced Line Chart Section */}
          <div className="h-[65vh] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Monthly performance based on settled invoices only</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="bg-slate-100 p-1 rounded-lg flex gap-1">
              <button onClick={() => setChartPeriod('daily')} className={`px-3 py-1 text-[10px] font-bold rounded-md ${chartPeriod === 'daily' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>Daily</button>
              <button onClick={() => setChartPeriod('weekly')} className={`px-3 py-1 text-[10px] font-bold rounded-md ${chartPeriod === 'weekly' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>Weekly</button>
              <button onClick={() => setChartPeriod('monthly')} className={`px-3 py-1 text-[10px] font-bold rounded-md ${chartPeriod === 'monthly' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>Monthly</button>
            </div>
        ...
        <div className="p-6">
          <div className="flex items-baseline gap-4 mb-8">
            <span className="text-4xl font-black tracking-tighter text-slate-900">{propertySettings?.currency || '$'}{revenueTrendSeries.totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            <div className={cn("flex items-center gap-1 text-emerald-500 bg-emerald-50 px-2 py-1 rounded-full text-[11px] font-bold", {
              'text-emerald-500 bg-emerald-50': revenueTrendSeries.revenueChangePercentage >= 0,
              'text-rose-500 bg-rose-50': revenueTrendSeries.revenueChangePercentage < 0,
            })}>
              <ArrowUpRight size={14} /> {revenueTrendSeries.revenueChangePercentage.toFixed(1)}%
            </div>
          </div>
          ...
          <div className="relative h-64 w-full">
            <svg viewBox="0 0 1000 220" className="w-full h-full overflow-visible">
              ...
              {(() => {
                const revenueData = revenueTrendSeries.revenueTrend.map(val => ({ val }));
                if(!revenueData.length) return null;
                const maxRev = Math.max(...revenueData.map(d => d.val), 1);
                const smoothPath = generateSmoothPath(revenueData);
                ...
                return (
                  <>
                    ...
                    {/* Area Fill */}
                    <path 
                      d={`${smoothPath} L 1000,200 L 0,200 Z`} 
                    ...
                    {/* Current Period Marker (Center Point) */}
                    {(() => {
                      const centerIndex = Math.floor(revenueData.length / 2);
                      const d = revenueData[centerIndex];
                      const x = (centerIndex / (revenueData.length - 1)) * 1000;
                      const y = 200 - (d.val / (maxRev * 1.2)) * 200;
                      return (
                        <>
                          {/* Vertical Guide Line */}
                          <line x1={x} y1="10" x2={x} y2="190" stroke="#f59e0b" strokeWidth="2" strokeDasharray="4,4" opacity="0.5" />
                          {/* Glow Circle */}
                          <circle cx={x} cy={y} r="16" fill="#f59e0b" opacity="0.15" />
                          {/* Main Marker */}
                          <circle cx={x} cy={y} r="10" fill="white" stroke="#f59e0b" strokeWidth="4" />
                          {/* Center Dot */}
                    ...
                    {/* Interactive Points */}
                    {revenueData.map((d, i) => {
                      const x = (i / (revenueData.length - 1)) * 1000;
                      const y = 200 - (d.val / (maxRev * 1.2)) * 200;
                      const centerIndex = Math.floor(revenueData.length / 2);
                      ...
                      return (
                        ...
                        >
                          <circle 
                            cx={x} 
                            cy={y} 
                            r={isHovered ? "10" : isCenter ? "8" : "6"} 
                            fill="white" 
                            stroke={isHovered ? "#3b82f6" : isCenter ? "#f59e0b" : "#2563eb"} 
                            strokeWidth={isHovered ? "5" : isCenter ? "4" : "3"} 
            ...
            {hoveredIndex !== null && (() => {
              const revenueData = revenueTrendSeries.revenueTrend.map(val => ({ val }));
              const x = (hoveredIndex / (revenueData.length - 1)) * 1000;
              const d = revenueData[hoveredIndex];
              const label = revenueTrendSeries.labels[hoveredIndex];
              
              // Calculate percentage change from previous period
              const prevVal = hoveredIndex > 0 ? revenueData[hoveredIndex - 1].val : d.val;
              const percentChange = prevVal > 0 ? ((d.val - prevVal) / prevVal) * 100 : 0;
              
              return (
                <div className="absolute top-2 left-1/2 transform -translate-x-1/2 pointer-events-none">
                  <div className="bg-slate-900 text-white rounded-lg shadow-2xl p-4 w-56 border border-slate-700">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                          {chartPeriod === 'daily' ? 'Daily' : chartPeriod === 'weekly' ? 'Weekly' : 'Monthly'} Revenue
                        </span>
                      </div>
                      <div className="text-2xl font-black text-white">
                        {propertySettings?.currency || '$'}{d.val.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                      <div className="flex items-center gap-2 pt-1">
                        <span className="text-[11px] font-medium text-slate-300">{label}</span>
                        <div className={cn("flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold", {
                          'text-emerald-400 bg-emerald-500/20': percentChange >= 0,
                          'text-rose-400 bg-rose-500/20': percentChange < 0,
                        })}>
            ...
            <div className="flex justify-between mt-4 px-2">
              {revenueTrendSeries.labels.map(label => (
                <span key={label} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{label}</span>
              ))}
            </div>
          </div>
        </div>
