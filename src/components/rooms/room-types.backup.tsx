...
import { collection, query, where, onSnapshot, doc as docRef } from 'firebase/firestore';
import { toast } from '@/hooks/use-toast';
import type { RoomType, BedType } from '@/types/roomType';
import { bedTypes } from '@/types/roomType';
...
export default function RoomTypesComponent({ propertyId }: RoomTypesComponentProps) {
  const { t } = useTranslation(['pages/rooms/types/content']);
  ...
  const calculateFormCompletion = () => {
    let filledFields = 0;
    const totalFields = 8; // Configurable count of important fields
          ...
          <DialogContent className="sm:max-w-3xl max-h-[90vh]">
            <ScrollArea className="max-h-[85vh] overflow-y-auto">
              <div className="space-y-0">
                ...
                <div className="relative bg-gradient-to-br from-blue-500 via-blue-400 to-cyan-400 dark:from-blue-900 dark:via-blue-800 dark:to-cyan-900 rounded-t-lg overflow-hidden">
                ...
                <div className="p-8 space-y-6">
                  ...
                  {viewingRoomType.description && (
                    <section className="space-y-3">
                      ...
                      <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/30 rounded-lg p-4 border border-purple-200 dark:border-purple-800/50">
                    ...
                    <section className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {viewingRoomType.assignedRoomNumbers.map(roomNum => (
                          <Badge 
                            key={roomNum}
                            className="bg-gradient-to-r from-orange-500 to-red-500 text-white border-0 px-4 py-2 text-base font-semibold"
                          >
                            🏠 {roomNum}
                          </Badge>
                        ))}
                    ...
                    <section className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {viewingRoomType.beds.map(bed => (
                          <div 
                            key={bed.type}
                            className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 rounded-lg p-4 border border-green-200 dark:border-green-800/50 hover:shadow-lg transition-shadow"
                          >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {defaultAmenities
                          ...
                          .map(amenity => (
                            <div 
                              key={amenity.id}
                              className="bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-950/30 dark:to-violet-950/30 rounded-lg p-3 border border-indigo-200 dark:border-indigo-800/50 hover:shadow-md transition-shadow flex flex-col items-center text-center"
                            >
                              ...
                              <span className="text-xs font-semibold text-foreground">{t(`amenities:${amenity.labelKey}`)}</span>
                            </div>
                          ))}
                      </div>
                    </section>
