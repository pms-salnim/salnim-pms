...
const drawTextWithHighlights = (doc: jsPDF, text: string, partsToHighlight: string[], x: number, y: number, maxWidth: number) => {
    ...
    lines.forEach((line: string) => {
        let currentX = x;
        const parts = line.split(splitRegex).filter(Boolean); // Split and remove empty strings
    ...
    return yOffset - y; // Return the total height of the text block
};
...
export async function generateSalaryCertificatePdf(
  ...
  yOffset += drawTextWithHighlights(doc, textWithoutTags(content.grossSalary), highlights, margin + 10, yOffset, contentWidth - 10);
  yOffset += 7;

  yOffset += drawTextWithHighlights(doc, textWithoutTags(content.netSalary), highlights, margin + 10, yOffset, contentWidth - 10);
  yOffset += 7;
